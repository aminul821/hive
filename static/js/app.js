...
function consumerQRCard(b){
 ...
 return `<div class="card"><div class="section-head"><h2>${esc(b.product)}</h2><span class="badge low">QR READY</span></div><div class="split"><div><div class="token">${esc(link)}</div><p class="muted">Use this official deep-link in the bottle QR. It opens the HiveTrust website's public preview.</p><div class="grid">${card("Bottle",b.token,"Unique token")}${card("Batch",b.batch,"Public")}${card("Origin",b.origin,"Public")}${card("Status",b.status,"Registry")}</div></div><div style="display:flex;justify-content:center;align-items:center"><div><img src="/api/qr/${encodeURIComponent(b.token)}" alt="Bottle QR code" style="width:150px;height:150px;border-radius:8px;background:#fff;padding:8px;display:block" onerror="this.style.display='none';this.nextElementSibling.style.display='block'"/><div class="qr" style="display:none">QR<br>${esc(b.token.slice(-8))}</div><div class="muted" style="margin-top:8px;text-align:center">Scan to verify</div></div></div></div><button class="btn" style="margin-top:10px" onclick="openBottleToken('${esc(b.token)}')">Open Consumer Preview</button></div>`;
}
